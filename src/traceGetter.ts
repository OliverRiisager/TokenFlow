...
async function getGethTrace(web3: Web3, txhash: string): Promise<GethTrace> {
    try {
        let callObjectData = await (web3 as any).debug.traceTransaction(
            txhash,
            {reexec: 5000, tracer: tracer}
        );
        let callObject: CallObject = CallObjectConvert.toCallObject(
            JSON.stringify(callObjectData)
        );
        let receiptData = await web3.eth.getTransactionReceipt(txhash);
        let receipt: Receipt = ReceiptConvert.toReceipt(
